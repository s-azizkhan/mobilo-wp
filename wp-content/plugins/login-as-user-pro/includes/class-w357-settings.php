...
class LoginAsUser_settingsPro {
	...
	public function __construct( $plugin_name, $version ) {
		...
		$this->fields = new LoginAsUser_fieldsPro();
	}
	...
	{
		add_options_page( 
			esc_html__( 'Login as User settings', 'login-as-user-pro'),
			'Login as User',
			'manage_options', 
			'login-as-user-pro',
	...
	{ 
		...
		{
			$message = __('Option saved successfully!', 'login-as-user-pro');
			$type = 'updated';
		...
		else 
		{
			...
			$message  = __('Error. The URL is not valid: ' . home_url('/') . $fields['redirect_to'] . '.', 'login-as-user-pro');
			$message .= (substr($redirect_to, 0, 1) == '/' || substr($redirect_to, 0, 1) == '\\') ? __('<br>Please remove the slash in front of URL.', 'login-as-user-pro') : '';
			$message .= (substr($redirect_to, 0, 8) == 'wp-admin') ? __('<br>You can\'t redirect the user to the admin page.', 'login-as-user-pro') : '';
			$message .= (substr($redirect_to, 0, 8) == 'wp-login') ? __('<br>You can\'t redirect the user to the login page.', 'login-as-user-pro') : '';
			$type = 'error';
		}
	...
	{
		$link = 'options-general.php?page=' . 'login-as-user-pro';
		$settings_link = '<a href="'.esc_url($link).'">'.esc_html__( 'Settings', 'login-as-user-pro' ).'</a>';
	...
	public function settings_init() 
	{
		 ...
		 * REGISTER SETTINGS
		 */
		register_setting( 'login-as-user-pro', 'login_as_user_options', array($this, 'validateSettings'));
		...
		add_settings_field( 
			'redirect_to', 
			__( 'URL Redirection', 'login-as-user-pro' ),
			...
			[
				...
				'size' => 60,
				'maxlength' => 250,
				'placeholder' => __('example: my-account/orders', 'login-as-user-pro'),
				'desc' => __('This is the URL path that you (Admin) will be redirected after logging in as a User.<br> For example: maybe you want to be redirected in user\'s orders page to see if an order has been completed successfully or cancelled, or just editing his profile details.<br>Leave it blank if you would like to be redirected to the home page (default)', 'login-as-user-pro'),
		...
		add_settings_field( 
			'login_as_type', 
			esc_html__( '"Login as...«option»" button', 'login-as-user-pro' ), 
			...
			[
				...
				'options' => [
					['id' => '0', 'label' => esc_html__( 'Nickname (username)', 'login-as-user-pro' ), 'value' => 'user_login'],
					['id' => '1', 'label' => esc_html__( 'First name', 'login-as-user-pro' ), 'value' => 'user_firstname'],
					['id' => '2', 'label' => esc_html__( 'Last name', 'login-as-user-pro' ), 'value' => 'user_lastname'],
					['id' => '3', 'label' => esc_html__( 'Full name (first & last)', 'login-as-user-pro' ), 'value' => 'user_fullname'],
					['id' => '4', 'label' => esc_html__( 'None (display only the user icon)', 'login-as-user-pro' ), 'value' => 'only_icon'],
				],
				'desc' => __('Choose which string will be displayed on the "Login as User" button.<br>For example Login as «Yiannis», or Login as «Christodoulou», or Login as «Johnathan99», or Login as «Yiannis Christodoulou».', 'login-as-user-pro'),
			]
		);
		...
		add_settings_field( 
			...
			esc_html__( 'Show only the first X characters on the "Login as...«option»" button', 'login-as-user-pro' ), 
			array($this->fields, 'selectField'),
			'login-as-user-pro', 
			'base_settings_section',
			[
				...
				'default_value' => '0',
				'options' => [
					['id' => '0', 'label' => esc_html__('All characters (default)', 'login-as-user-pro'), 'value' => '0'],
				...
				'desc' => __('Show only the first X characters of the username, or first/last name, or full name, on the "Login as...«option»" button. <br>For example, if you choose 5, the button will be displayed as Login as «Yiann...», or Login as «Chris...», or Login as «Johna...», or Login as «Yiann...».', 'login-as-user-pro'),
			]
		);
		...
		add_settings_field( 
			...
			esc_html__( 'Toolbar\'s position', 'login-as-user-pro' ), 
			array($this->fields, 'selectField'),
			'login-as-user-pro', 
			'base_settings_section',
			[
				...
				'default_value' => 'top',
				'options' => [
					['id' => 'top', 'label' => esc_html__('Top', 'login-as-user-pro'), 'value' => 'top'],
					['id' => 'bottom', 'label' => esc_html__('Bottom', 'login-as-user-pro'), 'value' => 'bottom'],
				],
				'desc' => __('Choose the position of the "Login as user" toolbar.', 'login-as-user-pro'),
			]
		);
		...
		add_settings_field( 
			'license_key', 
			esc_html__( 'License Key', 'login-as-user-pro' ),
			array($this->fields, 'textFieldForWeb357LicenseKey'),
