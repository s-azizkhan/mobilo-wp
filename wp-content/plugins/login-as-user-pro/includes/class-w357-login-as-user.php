		...
		switch ($_REQUEST['action']) {
			...
			case 'login_as_user':
				...
				if (!current_user_can('login_as_user', $user_id)) {
					wp_die(esc_html__('Could not login as user.', 'login-as-user-pro'));
				}
			...
			case 'login_as_olduser':
				...
				if ($this->login_as_user($old_user->ID, self::remember_me(), false)) {
					...
					wp_die(esc_html__('Could not switch users.', 'login-as-user-pro'));
				}
				break;
		}
	}
	...
	public function filter_login_message($message)
	{
		...
		if ($old_user instanceof WP_User) {
			$link = sprintf(
				...
				__('go back to admin as %1$s (%2$s)', 'login-as-user-pro'),
...
CSS;
			...
			wp_enqueue_style( 'login-as-user-pro', plugin_dir_url( dirname( __FILE__ ) ) . 'public/css/public.min.css', array(), LOGINASUSER_VERSION, 'all' );
			wp_enqueue_script( 'login-as-user-pro', plugin_dir_url( dirname( __FILE__ ) ) . 'public/js/public.min.js', array( 'jquery' ), LOGINASUSER_VERSION, false );
			...
			echo  '<div class="login-as-user-msg">'.sprintf(__('You have been logged in as the user <strong>%1$s</strong>', 'login-as-user-pro'), esc_html__($current_user_name)).'</div>';
	...
	public function login_as_type($user, $allow_trim_name = true)
	{
		...
		if (!empty($options->login_as_type))
		{
			...
			{
				case 'user_login':
					$login_as_type = esc_html__($user->user_login, 'login-as-user-pro');
					break;
					
				case 'user_firstname':
					$login_as_type = (!empty($user->user_firstname)) ? esc_html__($user->user_firstname, 'login-as-user-pro') : esc_html__($user->user_login, 'login-as-user-pro');
					break;

				case 'user_lastname':
					$login_as_type = (!empty($user->user_lastname)) ? esc_html__($user->user_lastname, 'login-as-user-pro') : esc_html__($user->user_login, 'login-as-user-pro');
					break;

				case 'user_fullname':
					$login_as_type = (!empty($user->user_firstname) || !empty($user->user_lastname)) ? esc_html__($user->user_firstname . ' ' . $user->user_lastname, 'login-as-user-pro') : esc_html__($user->user_login, 'login-as-user-pro');
					break;
	...
	public function login_as_user_metabox()
	{
		...
		if (get_current_user_id() != $user->ID)
		{
			...
			{
				$loginasuser_btn = __('No user exists for this order.', 'login-as-user-pro');
			}
			else
			{
				$loginasuser_btn = '<a class="button w357-login-as-user-btn w357-login-as-user-metabox-btn" href="' . esc_url($login_as_user_url) . '" title="'.esc_html__('Login as', 'login-as-user-pro').': ' . $this->login_as_type($user, false) . '"><span class="dashicons dashicons-admin-users"></span> '.esc_html__('Login as', 'login-as-user-pro').': <strong>' . $this->login_as_type($user) . '</strong></a>';
			}

			echo $loginasuser_btn;
		}
		else
		{
			if (!current_user_can('login_as_user', $user_id)) {
				echo __('Could not login as this user.', 'login-as-user-pro');
			}
			else
			{
				echo __('You are already logged in.', 'login-as-user-pro');
			}
		}	
	...
	public function loginasuser_col_content($val, $column_name, $user_id)
	{
		...
		switch ($column_name) {
			case 'loginasuser_col':
				...
				if (!current_user_can('login_as_user', $user_id)) {
					return __('Could not login as this user.', 'login-as-user-pro');
				}
				
				if (!$login_as_user_url || empty($user->user_login)) 
				{
					return __('Already logged in.', 'login-as-user-pro');
				}

				$options = (object) get_option( 'login_as_user_options' );
				if (!empty($options->login_as_type) && $options->login_as_type == 'only_icon')
				{
					return '<a class="button w357-login-as-user-btn w357-login-as-user-col-btn" href="' . esc_url($login_as_user_url) . '" title="'.esc_html__('Login as', 'login-as-user-pro').': ' . $this->login_as_type($user, false) . '"><span class="dashicons dashicons-admin-users"></span></a>';
				}

				return '<a class="button w357-login-as-user-btn w357-login-as-user-col-btn" href="' . esc_url($login_as_user_url) . '" title="'.esc_html__('Login as', 'login-as-user-pro').': ' . $this->login_as_type($user, false) . '"><span class="dashicons dashicons-admin-users"></span> '.esc_html__('Login as', 'login-as-user-pro').': <strong>' . $this->login_as_type($user) . '</strong></a>';		
	...
	public function loginasuser_woo_col_content($column)
	{
		if ('loginasuser_col' === $column) 
		{
			...
			if (get_current_user_id() != $user->ID)
			{
				...
				if (!$login_as_user_url || empty($user->user_login)) 
				{
					$html = __('No user exists for this order.', 'login-as-user-pro');
				}
				else
				{
					...
					if (!empty($options->login_as_type) && $options->login_as_type == 'only_icon')
					{
						$html = '<a class="button w357-login-as-user-btn w357-login-as-user-woo-col-btn" href="' . esc_url($login_as_user_url) . '" title="'.esc_html__('Login as', 'login-as-user-pro').': ' . $this->login_as_type($user, false) . '"><span class="dashicons dashicons-admin-users"></span></a>';
					}
					else
					{
						$html = '<a class="button w357-login-as-user-btn w357-login-as-user-woo-col-btn" href="' . esc_url($login_as_user_url) . '" title="'.esc_html__('Login as', 'login-as-user-pro').': ' . $this->login_as_type($user, false) . '"><span class="dashicons dashicons-admin-users"></span> '.esc_html__('Login as', 'login-as-user-pro').': <strong>' . $this->login_as_type($user) . '</strong></a>';
					}
				}
			}
			else
			{
				if (!current_user_can('login_as_user', $user_id)) {
					$html = __('Could not login as this user.', 'login-as-user-pro');
				}
				else
				{
					$html = __('You are already logged in.', 'login-as-user-pro');
				}
			}
	...
	function loginasuser_col($columns)
	{
		...
		foreach ($columns as $column_name => $column_info) {
			...
			if ('username' === $column_name || 'order_number' === $column_name || 'order_title' === $column_name) {
				$new_columns['loginasuser_col'] = __('Login as User', 'login-as-user-pro');
			}
		}
	...
	{
		$secure_auth_cookie    = w357LoginAsUserPro::secure_auth_cookie();
		$secure_olduser_cookie = w357LoginAsUserPro::secure_olduser_cookie();
		$secure_back_url_cookie = w357LoginAsUserPro::secure_back_url_cookie();
	...
	public function login_as_user_clear_olduser_cookie($clear_all = true)
	{
		...
		if ($clear_all || empty($auth_cookie)) {
			...
			if (w357LoginAsUserPro::secure_auth_cookie()) {
				$scheme = 'secure_auth';
			} else {
				$scheme = 'auth';
			}
	...
	public function login_as_user($user_id, $remember = false, $set_old_user = true)
	{
		...
		if (function_exists('WC')) {
			w357LoginAsUserPro::forget_woocommerce_session(WC());
		}
	...
	function loginasuser_individual_btn($user_data)
	{
		...
		return ('<a class="button w357-login-as-user-btn w357-login-as-user-woo-individual-btn" href="' . esc_url($login_as_user_url) . '" title="'.esc_html__('Login as', 'login-as-user-pro').': ' . $this->login_as_type($user, false) . '"><span class="dashicons dashicons-admin-users"></span> individual '.esc_html__('Login as', 'login-as-user-pro').': <strong>' . $this->login_as_type($user) . '</strong></a>');
	}
	...
	function loginasuserShortcode($atts)
	{
		...
		// Get the user_id from the shortcode attributes
		if ($user_id === 0)
		{
			echo '<p>The user_id is not specified</p>';
			return ob_get_clean();
		}
		...
		if (get_current_user_id() != $user->ID)
		{
			...
			if (!$login_as_user_url || empty($user->user_login)) 
			{
				$loginasuser_btn = __('Could not login as this user.', 'login-as-user-pro');
			}
			else
			{
				$loginasuser_btn = '<a class="button w357-login-as-user-btn w357-login-as-user-metabox-btn" href="' . esc_url($login_as_user_url) . '" title="'.esc_html__('Login as', 'login-as-user-pro').': ' . $this->login_as_type($user, false) . '">'.esc_html__('Login as', 'login-as-user-pro').': <strong>' . $this->login_as_type($user) . '</strong></a>';
			}
		}
		else
		{
			if (get_current_user_id() == $user->ID)
			{
				$loginasuser_btn = __('You are already logged in.', 'login-as-user-pro');
			}
			elseif (!current_user_can('login_as_user', $user_id)) {
				$loginasuser_btn = __('Could not login as this user.', 'login-as-user-pro');
			}
			else
			{
				$loginasuser_btn = __('You cannot login as this user.', 'login-as-user-pro');
			}
		}	

		if (!empty($loginasuser_btn))
		{
			echo "<div>".$loginasuser_btn."</div>";
		}
