	...
	class WC_Smart_Coupons {
		...
		public function generate_coupon_description( $args = array() ) {
			try {
				...
				$this->sc_block_catch_error( $e );
				return __( 'Coupon details unavailable at the moment.', 'woocommerce-smart-coupons' );
			}
		}
		...
		public function export_coupon_csv( $columns_header, $data ) {
			try {
				...
				$this->sc_block_catch_error( $e );
				return array( 'error' => __( 'Failed to generate export CSV due to internal error.', 'woocommerce-smart-coupons' ) );
			}
		...
		public static function insert_setting_after( &$settings, $insert_after_setting_id, $new_setting, $insert_type = 'single_setting' ) {
			try {
				...
				if ( method_exists( __CLASS__, 'log' ) ) {
					self::log( 'error', '[Smart Coupons] Error in insert_setting_after: ' . $e->getMessage() . ' ' . __FILE__ . ' ' . __LINE__ );
				}
			}
		}
		...
		public function get_cookie_life() {
			try {
				...
				$this->sc_block_catch_error( $e );
				return apply_filters( 'wc_sc_coupon_cookie_life', time() + ( 60 * 60 * 24 * 180 ) );
			}
		...
		public static function get_smart_coupons_plugin_data() {
			try {
				...
				if ( method_exists( __CLASS__, 'log' ) ) {
					self::log( 'error', '[Smart Coupons] Error in get_smart_coupons_plugin_data: ' . $e->getMessage() . ' ' . __FILE__ . ' ' . __LINE__ );
				}
				return array();
			}
		}
    ...
    color: #30050b;
		...
		public function convert_gift_sending_date_time_to_timestamps( $request_gift_sending_date_time ) {
			try {
				...
				return new WP_Error(
					'conversion_error',
					_x( 'An unexpected error occurred while converting gift sending date & time.', 'Error message', 'woocommerce-smart-coupons' )
				);
			}
		}
