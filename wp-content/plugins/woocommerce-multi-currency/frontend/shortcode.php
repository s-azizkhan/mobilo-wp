...
class WOOMULTI_CURRENCY_Frontend_Shortcode {
	...
	public function shortcode_plain_vertical( $atts, $content = null ) {
		...
		?>
        <div id="<?php echo esc_attr( self::get_shortcode_id() ) ?>"
             class="woocommerce-multi-currency wmc-shortcode plain-vertical <?php echo esc_attr( $class ) ?>"
            ...
            <div class="wmc-currency-wrapper">
				<span class="wmc-current-currency" style="line-height: <?php echo esc_attr( $line_height ) ?>">
                    ...
                    if ( $custom_format ) {
	                    ...
	                    echo "<span class='wmc-text wmc-text-{$current_currency}'><span class='wmc-text-currency-text'>({$current_currency}) </span>{$symbol}</span>";
                    }
                    ?>
                    </span>
                    <?php
