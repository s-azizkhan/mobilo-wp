...
// https://buy.dev.mobilocard.com/?token=936969b26e94a782fe3771b261c79144
...
add_action('wp_loaded', function(){
    if (isset($_COOKIE['auto_gift_accessories_product_added'])) {
        return; // Only apply if the cookie exists
    } {
        add_free_gift_product_to_cart();
        setcookie('auto_gift_accessories_product_added', '1', time() + 600, '/');
    }
});
        ...
        foreach ($free_sku as $sku) {
            ...
            foreach (WC()->cart->get_cart() as $cart_item) {
                if ($cart_item['product_id'] == $gift_product_id) {
                    ...
                    if (!isset($cart_item['promotional_free_gift'])) {
                        $cart_item['promotional_free_gift'] = true;
                    }
                    continue;
                }
            }
            ...
            WC()->cart->add_to_cart($gift_product_id, 1, 0, array(), array('promotional_free_gift' => true));
        }
...
function set_gift_product_price($cart)
{
    try {
        foreach ($cart->get_cart() as $cart_item_key => $cart_item) {
            if (isset($cart_item['promotional_free_gift']) && $cart_item['promotional_free_gift']) {
...
function clear_gift_cookie()
{
    ...
    if (isset($_COOKIE['free_shipping_active'])) {
        setcookie('gift_accessories_product_active', '', time() - 3600, '/'); // Expire the cookie
    }
}
