...
add_action('wp_footer', 'fix_pay_button_on_all_checkout_errors', 20);
function fix_pay_button_on_all_checkout_errors() {
    ...
    jQuery(function($) {
        ...
        // Initial error reset
        if ($('.woocommerce-error, .woocommerce-invalid').length > 0) resetButton();

        // On click — change text and disable with delay
        ...
        // Hook into WooCommerce's error event (most reliable)
        $(document.body).on('checkout_error', function() {
            resetButton();
        });

        // Fallback observer — catch any other dynamic DOM errors
        const targetForm = document.querySelector('form.woocommerce-checkout, form#order_review');
        if (targetForm) {
            new MutationObserver(() => {
                if ($('.woocommerce-error, .woocommerce-invalid').length > 0) {
                    resetButton();
                }
            }).observe(targetForm, {
                childList: true,
