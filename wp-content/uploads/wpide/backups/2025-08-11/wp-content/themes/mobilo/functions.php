...
add_action( 'template_redirect', function() {
    ...
    // --- Auto Logout on order-received (delay until shutdown for saved payment methods to be stored) ---
    if ( function_exists( 'is_order_received_page' ) && is_order_received_page() && is_user_logged_in() ) {
        ...
        if ( $order_id && ( $order = wc_get_order( $order_id ) ) && $order->is_paid() ) {
            ...
            if ( function_exists( 'WC' ) && WC()->session ) {
                ...
                if ( $flag_order_id && intval( $flag_order_id ) === $order_id ) {
                    ...
                    // Schedule logout + redirect after page render so WooCommerce can save tokens
                    add_action( 'shutdown', function() {
                        if ( function_exists( 'WC' ) && WC()->session ) {
                            WC()->session->destroy_session();
                        }
                        ...
                        // change this to your custom thank-you page
                        wp_safe_redirect( home_url( '/payment-done/' ) );
                        exit;
                    });
                }
            }
...
/**
 * 2) Render editable billing fields inside the order-pay form (before the payment methods)
 *    Hook into form-pay template using the hook that exists inside the <form> (so fields POST with payment)
 */
add_action( 'woocommerce_pay_order_before_payment', function() {

    // Only run when pay_for_order key present (safety)
    ...
    // Checkout instance & billing fields
    $checkout = WC()->checkout();
    $fields   = $checkout->get_checkout_fields( 'billing' );

    // Output billing block (similar to form-billing.php)
    echo '<div class="woocommerce-billing-fields order-pay-billing-fields">';
    ...
    foreach ( $fields as $key => $field ) {
        ...
        // Try order getters first (prefill from order)
        $value = '';
        $getter = 'get_' . $key;
        if ( method_exists( $order, $getter ) ) {
            $value = $order->{$getter}();
        } else {
            // fallback to order meta (some older keys)
    ...
    echo '</div>'; // .woocommerce-billing-fields__field-wrapper

    do_action( 'woocommerce_after_checkout_billing_form', $checkout );

    // Add "Update billing & continue to payment" button (AJAX)
    // We'll also output the order id and a nonce for security.
    ?>
    <div style="margin:20px 0;">
        <button type="button" id="wc-order-pay-update-billing" class="button"><?php esc_html_e( 'Update billing and show payment methods', 'woocommerce' ); ?></button>
        <span id="wc-order-pay-update-status" style="margin-left:8px;"></span>
    </div>

    <input type="hidden" id="wc-order-pay-order-id" value="<?php echo esc_attr( $order_id ); ?>" />
    <?php wp_nonce_field( 'wc_update_order_billing', 'wc_update_order_billing_nonce' ); ?>

    </div><!-- .order-pay-billing-fields -->
    <?php
...
/**
 * 3) Enqueue JS only on order-pay page to handle AJAX save + reload
 */
add_action( 'wp_enqueue_scripts', function() {

    if ( function_exists( 'is_wc_endpoint_url' ) && is_wc_endpoint_url( 'order-pay' ) ) {

        // enqueue a small inline script (or you can enqueue an external file)
        wp_register_script( 'wc-order-pay-billing', false );
        wp_enqueue_script( 'wc-order-pay-billing' );

        $ajax_data = array(
            'ajax_url'   => admin_url( 'admin-ajax.php' ),
            'nonce'      => wp_create_nonce( 'wc_order_pay_billing_ajax' ),
            'i18n' => array(
                'saving' => __( 'Saving...', 'woocommerce' ),
                'done'   => __( 'Saved. Reloading…', 'woocommerce' ),
                'error'  => __( 'There was an error. Check console.', 'woocommerce' ),
            ),
        );

        wp_add_inline_script( 'wc-order-pay-billing', "
        (function($){
            var button = $('#wc-order-pay-update-billing');
            if (!button.length) return;

            button.on('click', function(e){
                e.preventDefault();
                var orderId = $('#wc-order-pay-order-id').val();
                var data = {
                    action: 'wc_update_order_billing',
                    order_id: orderId,
                    security: '{$ajax_data['nonce']}'
                };

                // collect billing inputs (inputs/selects/textarea whose name starts with billing_)
                $('#order_review').find('input[name^=\"billing_\"] , select[name^=\"billing_\"] , textarea[name^=\"billing_\"]').each(function(){
                    var name = $(this).attr('name');
                    data[name] = $(this).val();
                });

                $('#wc-order-pay-update-status').text('{$ajax_data['i18n']['saving']}');

                $.post('{$ajax_data['ajax_url']}', data, function(resp){
                    if ( resp && resp.success ) {
                        $('#wc-order-pay-update-status').text('{$ajax_data['i18n']['done']}');
                        // reload same page so payment methods / gateway JS get enqueued and rendered
                        setTimeout(function(){ location.href = location.href; }, 600);
                    } else {
                        $('#wc-order-pay-update-status').text('{$ajax_data['i18n']['error']}');
...
/**
 * 4) AJAX handler: validate nonce, update billing on the order (and user, if exists)
 */
add_action( 'wp_ajax_wc_update_order_billing', 'wc_update_order_billing_ajax' );
add_action( 'wp_ajax_nopriv_wc_update_order_billing', 'wc_update_order_billing_ajax' );

function wc_update_order_billing_ajax() {
    // basic nonce check (we used a custom nonce above)
    if ( empty( $_POST['security'] ) || ! wp_verify_nonce( wp_unslash( $_POST['security'] ), 'wc_order_pay_billing_ajax' ) ) {
    ...
    // Use WC_Checkout to get canonical billing fields list
    $checkout = WC()->checkout();
    $billing_fields = $checkout->get_checkout_fields( 'billing' );

    // Update order (use setters when available, otherwise meta)
    foreach ( $billing_fields as $key => $field ) {
        if ( isset( $_POST[ $key ] ) ) {
            $val = wc_clean( wp_unslash( $_POST[ $key ] ) );

            // setters on WC_Order are like set_billing_first_name()
            $setter = 'set_' . $key;
    ...
    // If order is attached to a WP user, optionally update their user meta
